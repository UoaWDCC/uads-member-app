...
function loadPluginFromDirectory(directory, options) {
    ...
    plugin.commands = ramda_1.map(command_loader_1.loadCommandFromPreload, options.preloadedCommands || []);
    ...
    if (jetpackPlugin.exists('commands') === 'dir') {
        ...
        plugin.commands = plugin.commands.concat(ramda_1.map(function (file) { return command_loader_1.loadCommandFromFile(directory + "/commands/" + file); }, commands));
    }
    ...
    if (jetpackPlugin.exists('extensions') === 'dir') {
        ...
        plugin.extensions = ramda_1.map(function (file) { return extension_loader_1.loadExtensionFromFile(directory + "/extensions/" + file); }, extensions);
    }
    else {
        plugin.extensions = [];
    }
